...
def find_work_done_ke(ke_formula, information_check_dict):
    ...
    if information_check_dict[required_information[0]] == "Yes":
        steps_response = steps_response + \
            "I understand that the work done is equal to the change in kinetic energy.\n"
...
def check_total_mechanical_energy_conserved(information_check_dict):
    ...
    if information_check_dict[required_information[1]] == "Yes":
        steps_response = steps_response + \
            "I understand that the total mechanical energy of an object is the sum of it's kinetic energy and potential energy.\n"
...
def evaluate(information_check_dict, formulas):
    ...
    correct = False
    steps_working, work_done_1 = find_work_formula(formulas)
    if work_done_1:
    ...
    if ke:
        working = working + steps_working
    try:
        steps_working, work_done_2 = find_work_done_ke(
    ...
    if work_done_1 and not work_done_2:
        try:
            working = working + \
                insert_latex(
                    "W = " + latex(work_done_1.subs([(s, values_dict["s_old"])]))) + "\n"
            working = working + \
                insert_latex(
                    "100 J = " + latex(work_done_1.subs([(s, values_dict["s_old"])]))) + "\n"
            ...
            working = working + \
                insert_latex(
                    "W = " + latex(work_done_1.subs([(s, values_dict["s_new"])]))) + "\n"
        ...
        try:
            answer = simplify(work_done_2.subs(
            ...
            if information_check_dict[required_information[4]] == "Yes":
                try:
                    ...
                    working = working + \
                        insert_latex(
                            "100 J = " + latex(work_done_1.subs([(s, values_dict["s_old"])]))) + "\n"
                    work_done_old = work_done_1.subs(
                        [(s, values_dict["s_old"])])
                    working = working + "Substituting " + \
                        insert_latex(latex(
                            values_dict["s_old"])) + " for " + insert_latex(latex(values_dict["s_new"])) + "\n"
                    working = working + \
                        insert_latex(
                            "W = " + latex(work_done_1.subs([(s, values_dict["s_new"])]))) + "\n"
                    work_done_new = work_done_1.subs(
                        [(s, values_dict["s_new"])])
                    answer = work_done_new / work_done_old * values_dict["W"]
                    working = working + \
                        insert_latex(
                            "W = " + '{:.2f}'.format(answer) + answer_unit)
